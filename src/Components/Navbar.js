...
function Navbar({ handleMenu, showMenu }) {
  ...
  return (
    <div>
      <nav className="backdrop-blur-sm h-16 md:h-20 flex justify-between items-center fixed left-0 right-0 bg-white/95 dark:bg-slate-900/95 z-50 transition-all duration-500">
        <Fade top delay={0}>
          <Link href="#" to="intro" spy={true} smooth={true} offset={0} duration={1000} className="ml-6 md:ml-12 flex flex-row ">
            <svg className="w-8 h-8 fill-black dark:fill-slate-200" viewBox="0 0 22 19" fill="none" xmlns="http://www.w3.org/2000/svg">
